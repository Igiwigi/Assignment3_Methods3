...
stat_summary(fun = "max", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red")+
geom_text(data = means, aes(label = paste("Mean:", round(mean_ASD, 2)), x = Diagnosis, y = mean_ASD), vjust = -7, hjust = -0.1, size = 4)+
...
training_set <- df[train_indices, -which(names(df) == "ID")]
testing_set <- df[-train_indices, -which(names(df) == "ID")]
#taking out ID from the test-training data as its too many factors for randomForest

#however ID might be very important for seeing which person is which as each individual does multiple studies?
#!!!
```
...
rf_model <- randomForest(Diagnosis ~ ., data = training_set, ntree = 100)

predictions <- predict(rf_model, newdata = testing_set)
```
```{r testing_simple_model_metrics}
conf_matrix <- confusionMatrix(predictions, testing_set$Diagnosis)
conf_matrix
...
roc_auc <- roc(testing_set$Diagnosis, as.numeric(predictions))$auc
...
#seems higher than chance but not much better
```
