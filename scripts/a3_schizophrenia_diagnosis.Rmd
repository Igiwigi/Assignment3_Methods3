...
df %>%
  group_by(Diagnosis) %>%
  summarize(Count = n())
#ratio between diagnoses seems balanced
...
#doing a quick and dirty t-test to "confirm" the assumptions made at a glance
df %>%
  group_by(Diagnosis) %>%
  summarise(across(where(is.numeric), ~ t.test(.x ~ as.factor(Diagnosis), data = df)$p.value))%>%
  select_if(function(x) any(x < 0.05))

#most columns are "statistically significant" as in, from different populations (this being a difference between the diagnoses)
...
#pause n and articulation rate were not significant in making the two groups different
```
...
ggplot(df, aes(x = Diagnosis, y = ASD, fill = Diagnosis)) +
  ...
  ggtitle("Number of Syllables Spoken by Diagnosis") +
      geom_text(data = means, aes(label = paste("mean:", round(ASD, 6))), vjust = -7, hjust = -0.1, size = 4) + 
  theme_minimal()
...
ggplot(df, aes(x = Diagnosis, y = nsyll, fill = Diagnosis)) +
  ...
  labs(x = "Diagnosis", y = "Number of Syllables") +
  ggtitle("Number of Syllables Spoken by Diagnosis") +
      geom_text(data = means, aes(label = paste("mean:", round(nsyll, 6))), vjust = -7, hjust = -0.1, size = 4) + 
  theme_minimal()
