...
pacman::p_load(tidyverse, ggplot2, dplyr, caret, parsnip, pROC, ranger, workflows, vip, yardstick, mlbench, tidymodels, keras)
```
...
#there appear to be some extreme values
df <- df %>% 
  mutate(across(
    where(is.numeric),
    ~ ifelse(
...
#68.31% accuracy at default? with no feature engineering?
```
Now doing it like in the lectures
```{r maxime-example}
#doing this like in the lectures

#setting a recipe
this_recipe <-
recipes::recipe(Diagnosis ~ ., data = training_set) %>%
...
as_tibble() %>%
mutate(
predict(rf_fit1, new_data = testing_set),
predict(rf_fit1, new_data = testing_set, type = "prob")
)
...
metrics_plot
#more or less the same accuracy obtained, 60ish
```
...
```{r feature_engineering_feature_removal_attempt}
...
```{r optimize_model}
model <- train(formula, data = training_set, method = "rf",
